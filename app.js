...
const LIGHT_BANDS = [
  { maxKg: 20, rates: [269, 269, 269, 293, 458, 543, 610, 672, 732, 763, 793, 823] },
  { maxKg: 40, rates: [329, 329, 329, 343, 506, 593, 660, 720, 780, 811, 842, 872] },
  { maxKg: 100, rates: [391, 391, 391, 415, 555, 641, 708, 768, 830, 859, 890, 922] },
  { maxKg: 400, rates: [439, 439, 439, 452, 604, 689, 756, 818, 878, 909, 939, 970] },
  { maxKg: 600, rates: [448, 460, 468, 498, 630, 711, 777, 885, 956, 1026, 1059, 1092] },
  { maxKg: 800, rates: [464, 502, 523, 589, 684, 753, 815, 922, 1006, 1149, 1184, 1220] },
  { maxKg: 999, rates: [489, 551, 605, 727, 764, 819, 873, 977, 1080, 1292, 1358, 1410] },
];

const HEAVY_PER_TON_RATES = [520, 624, 666, 788, 824, 879, 934, 1042, 1146, 1397, 1436, 1475];
const OTO_PER_TON_RATES = [367, 489, 539, 635, 788, 831, 854, 903, 1002, 1189, 1220, 1282];
...
function calcPayoutValue(type, distKm, weightKg) {
  ...
  if (isLongDistance(distKm)) {
    ...
    if (type === 'OTO') {
      const tons = Math.max(1, weightKg / 1000);
      raw = OTO_PER_TON_RATES[di] * tons;
